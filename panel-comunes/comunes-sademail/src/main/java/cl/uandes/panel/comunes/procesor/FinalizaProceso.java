...
public class FinalizaProceso implements Processor {
	...
	Object request;
	private final String operacionesInvocadasScheduler = "suspender_eliminar_cuentas|asignar_owners|crear_cuentas|crear_grupos|grupos_inprogress|grupos_inprogress_postgrado|crear_cuentas_AD|";
	...
	public void process(Exchange exchange) throws Exception {
		...
		try {
			...
			logger.info(String.format("ActualizaMiResultados: keyContador=%s DatosKcoFunciones datos=%s", getKeyContador(), datos));
			...
			if (esInvocadoPorScheduler(exchange)) {
				logger.info(String.format("Va a invocar al fin del scheduler: operacion=%s", operacion));
