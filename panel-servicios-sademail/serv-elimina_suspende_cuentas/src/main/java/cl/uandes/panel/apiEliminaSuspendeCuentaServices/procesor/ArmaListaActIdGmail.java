...
public class ArmaListaActIdGmail implements Processor {
	...
	public void process(Exchange exchange) throws Exception {
		...
		for (Map<String, Object> map : resultados) {
			...
			String loginName = URLEncoder.encode(((String)map.get("CUENTA")).trim(), java.nio.charset.StandardCharsets.UTF_8.toString());
			String url = String.format(uriRetrieveUser, loginName);
			logger.info(String.format("ArmaListaActId: loginName %s url: %s", loginName, url));
			...
			if (response.getStatus() < 300) {
				...
				if (userResponse != null && userResponse.getCodigo() == 0) {
					...
					logger.info(String.format("ArmaListaActId: ID_GMAIL %s ", map.get("ID_GMAIL")));
				}
			}
		}
