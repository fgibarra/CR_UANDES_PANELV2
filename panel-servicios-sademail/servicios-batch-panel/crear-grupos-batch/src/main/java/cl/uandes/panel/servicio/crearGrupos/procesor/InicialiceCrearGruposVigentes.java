 ...
 *
 */
public class InicialiceCrearGruposVigentes implements Processor {
...
/*
        try {
            ...
            logger.info("inicialiceGruposInprogress: Inicializa BD");
            Session hSession = HibernateHelper.getSession(thread.getName());
            tx = hSession.beginTransaction();
            funcion = dao.getFuncion(hSession, GRUPOS_INPROGRESS);
            String[] args = new String[1];
            args = ut.generaArray(funcion.getParametros().trim());
            CommandLineParser parser = new CommandLineParser(args);
            String disabled = parser.getValue("disabled");
         	logger.debug("inicialiceCrearCuentas: disabled="+disabled+" parameters="+funcion.getParametros());
            if (disabled != null && "true".equals(disabled.toLowerCase())) {
            	logger.debug("inicialiceGruposInprogress: funcion DISABLED!");
            ...
            if (funcion != null) {
                ...
                tx = hSession.beginTransaction();
                // poblar los datos
                logger.info("inicialiceGruposInprogress: poblar Gmail");

                HashMap<String, Integer> solicitados =
                    dao.pueblaInprogressGroupMembers(hSession,
                ...
                logger.info("inicialiceGruposInprogress: gruposSolicitados="+
                			resultadoFuncion.getGruposSolicitados()+
                			" miembrosSolicitados="+
                			resultadoFuncion.getGruposSolicitados());
                listaGrupos =
                        dao.getListaGruposInprogress(hSession, fuente);
                if (listaGrupos == null) {
                    // fallo la gracion en funcion pueblaGroupMembers
                    throw new RuntimeException("fallo la gracion en funcion pueblaGroupMembers");
                }
                logger.info("inicialiceGruposInprogress: listaGrupos.size="+listaGrupos.size());
                indxListaGrupo = 0;
